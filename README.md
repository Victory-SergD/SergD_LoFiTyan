...
- `src/` — фронтенд на Svelte: интерфейс, компоненты, локали, сторы.
  - `src/lib/stores/radio.ts` — радио (radio-browser.info, перебор зеркал + воспроизведение).
  - `src/lib/stores/` — также `volume`, `weather`, `immersion`, `fullscreen`.
  - `src/lib/components/` — `RadioPlayer`, `Controls` (атмосфера + ⛶ полный экран), `Canvas` (фон), `Settings`, `TopBar`.
